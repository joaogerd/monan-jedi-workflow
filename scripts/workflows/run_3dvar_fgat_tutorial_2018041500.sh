...
append_workflow_step "load_site_environment" "source scripts/env/load_jaci_env.sh ${SITE_ENV}"
...
if [[ "${CLEAN_RUNTIME}" == true ]]; then
  append_workflow_step "clean_runtime" "rm -rf ${RUNTIME_DIR}" "optional"
...
append_workflow_step "render_jedi_yaml" "bash scripts/run/render_3dvar_fgat.sh"
...
append_workflow_step "prepare_runtime" "bash scripts/run/prepare_3dvar_fgat_runtime.sh --strict"
log_info "Preparing runtime"
bash scripts/run/prepare_3dvar_fgat_runtime.sh --strict

if [[ "${EXECUTE}" == true ]]; then
  append_workflow_step "run_variational" "bash scripts/run/run_3dvar_fgat_variational.sh --execute" "execute"
  log_info "Preparing variational command and executing"
  bash scripts/run/run_3dvar_fgat_variational.sh --execute
else
  append_workflow_step "prepare_variational_command" "bash scripts/run/run_3dvar_fgat_variational.sh" "dry-run"
  log_info "Preparing variational command"
  bash scripts/run/run_3dvar_fgat_variational.sh
fi

if [[ "${SUBMIT_PBS}" == true ]]; then
  append_workflow_step "render_pbs_job" "bash scripts/run/render_3dvar_fgat_pbs.sh" "pbs"
  ...
  append_workflow_step "submit_pbs_job" "qsub ${PBS_FILE}" "pbs"
  log_info "Submitting PBS job: ${PBS_FILE}"
  qsub "${PBS_FILE}"
else
  append_workflow_step "render_pbs_job" "bash scripts/run/render_3dvar_fgat_pbs.sh" "skipped"
