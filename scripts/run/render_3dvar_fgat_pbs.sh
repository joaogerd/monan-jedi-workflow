...
site_env="${SITE_ENV:-${REPO_ROOT}/configs/sites/jaci/site.env}"
...
if [[ -f "${site_env}" ]]; then
  ...
  log_warn "Site environment file not found while rendering PBS: ${site_env}"
fi
...
command:
  executable: python3
  argv: ${REPO_ROOT}/tools/render_template.py ${template_file} --context ${context_file} --output ${output_file} --allow-env
outputs:
  pbs_script: ${output_file}
notes:
  - This script only renders PBS.
  - qsub is intentionally outside this stage.
  - Rendering is strict; unresolved placeholders fail before submission.
EOF
